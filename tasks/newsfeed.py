...
class CrawlNewsfeed(Base):
    ...
    def handle_crawl_web(self, driver, stop_event):
        ...
        while time() - start_time < time_run and not stop_event.is_set():
            try:
                ...
                if (
                    filter_keyword
                    and now - last_run_search >= timeout_search
                    and len(keywords) > 0
                ):
                        ...
                        if idAreaPost not in listId:
                            ...
                            for link in links:
                                ...
                                if (
                                    ...
                                    try:
                                        converTime = self.convert_to_db_format(
                                            link_time
                                        )
                                    except:
                                        converTime = None
                                    ...
                                    numberOfReactionsAndComments = (
                                        self.get_number_of_reactions_and_comments(
                                            driver, modal
                                        )
                                    )
                                    ...
                                    print(
                                        "numberOfReactionsAndComments: ",
                                        json.dumps(
                                            numberOfReactionsAndComments, indent=4
                                        ),
                                    )
                                    # Lấy bình luận
                                    ...
                                    self.histories.update(
                                        id=history_id, data={"counts": quantity_post}
                                    )
                ...
                for modal in list_posts:
                    ...
                    try:
                        ...
                        if idAreaPost not in listId:
                            ...
                            btn_comment = driver.find(
                                xpaths.btn_comment_article_mobile, parent=modal
                            )
                            if btn_comment is None:
                                continue
                            driver.click_script(btn_comment)

                            time_up_element = driver.find(
                                xpaths.time_up_mobile, parent=modal
                            )
                    ...
                    if idAreaPost not in listId:
                        ...
                        for link in links:
                            ...
                            if (
                                ...
                                print(
                                    "numberOfReactionsAndComments: ",
                                    json.dumps(numberOfReactionsAndComments, indent=4),
                                )
                                # Lấy bình luận
                                comments, has_link_in_comments = self.get_comments(
                                    modal, driver
    ...
    # lấy số lượng cảm xúc và bình luận pc
    ...
    def action_get_number_of_reactions_and_comments(
        self, locator_all_reactions, locator_comment, locator_shares
    ):
        data = {
            "comment": 0,
            "like": 0,
        ...
        if locator_all_reactions is not None:
            like = locator_all_reactions.inner_text()
            like = self.convert_shorthand_to_number(like)
            data["like"] = like
        if locator_comment is not None:
            comment = locator_comment.inner_text()
            comment = self.convert_shorthand_to_number(comment)
            data["comment"] = comment
        if locator_shares is not None:
            shares = locator_shares.inner_text()
    ...
    def get_comments(self, modal, driver):
        ...
        try:
            ...
            if len(comments) == 0:
                print(
                    "Khong lay duoc comments bang cach thuong, su dung fallback method..."
                )
                data, has_link_in_comments = self._parse_comments_from_html(
                    driver, modal
                )
        ...
        try:
            ...
            for xpath in xpaths.comments:
                ...
                if len(comment_elements) > 0:
                    print(
                        f"Tim thay {len(comment_elements)} comments bang XPath: {xpath}"
                    )
                    break
            ...
            for cm_element in comment_elements[:10]:  # Giới hạn 10 comments
                try:
                    ...
                    textComment = " ".join(
                        [t.strip() for t in text_content if t.strip()]
                    )
                    ...
                    textContentComment = textContentComment.replace(
                        "Follow", ""
                    ).strip()
